...
const certifications = [
  { name: 'Internet and Computing Core Certification (IC3)', issuer: 'CERTIPORT', year: '2015' },
  { name: 'Ethical Hacking (CEH)', issuer: 'EC-Council', year: '2023' },
  { name: 'Diplomado Design Thinking', issuer: 'UPLA', year: '2024' },
  { name: 'Diplomado Growth Hacking', issuer: 'UPLA', year: '2024' },
  { name: 'Diplomado Programación JavaScript', issuer: 'UPLA', year: '2024' },
  { name: 'Diplomado Gestión de Proyectos Ágiles', issuer: 'UPLA', year: '2024' },
  { name: 'Maestría en Administración de Bases de Datos SQL', issuer: 'Track IT', year: '2010' },
  { name: 'Maestría en Programación de Bases de Datos SQL', issuer: 'Track IT', year: '2010' }
]

const cursosSENCE = [
  'Aplicar Excel En La Industria (Código: 1237848206)',
  'Aplicación Herramientas De Excel En El Mundo Laboral (1237876495)',
  'Manejo De Macros Y Tablas Dinámicas En Excel (1237887134)',
  'Experto en Ciberseguridad (2103010005)'
]
...
<template>
  <section class="py-20 px-4 bg-gradient-to-b from-gray-900 to-black text-white relative overflow-hidden">
    <!-- Fondo de partículas -->
    ...
    <div class="container mx-auto relative z-10">
      ...
      <div class="text-center mb-12">
        ...
        <p class="text-gray-400 max-w-2xl mx-auto font-mono">
          > STACK COMPLETO • CERTIFICACIONES • HABILIDADES_
        </p>
      </div>

      <!-- Categorías tabs -->
      <div class="flex flex-wrap gap-2 justify-center mb-8">
        <button v-for="category in skillCategories" :key="category.name"
                @click="activeCategory = category"
                class="px-4 py-2 rounded-full font-mono text-sm transition-all duration-300 cyber-card"
                :class="activeCategory.name === category.name ? 'bg-cyan-500/20 text-cyan-400 border-cyan-500' : 'bg-black/40 text-gray-400 border-gray-700'">
      ...
      <div class="grid grid-cols-2 md:grid-cols-3 gap-4 mb-12">
        <div v-for="skill in activeCategory.skills" :key="skill.name"
          ...
          <div class="flex justify-between items-center mb-2">
            ...
            <span class="text-xs font-mono" :class="`text-${skill.color}-400`">{{ skill.level }}%</span>
          </div>
        ...
        <div class="cyber-card bg-black/40 p-6">
          ...
          <div class="space-y-3">
            <div v-for="cert in certifications" :key="cert.name"
                 class="flex items-start gap-3 p-3 bg-black/60 rounded-lg border border-cyan-500/10">
              <span class="text-purple-400">⚡</span>
              <div>
                <p class="text-sm font-bold text-white">{{ cert.name }}</p>
                <p class="text-xs text-gray-400">{{ cert.issuer }} • {{ cert.year }}</p>
              </div>
            </div>
          </div>
        </div>

        <!-- Cursos SENCE y Soft Skills -->
        <div class="space-y-6">
          <div class="cyber-card bg-black/40 p-6">
            <h3 class="text-xl font-bold text-purple-400 mb-4 font-mono flex items-center gap-2">
              <span>📚</span> CURSOS SENCE (APROBADOS)
            </h3>
            <ul class="space-y-2">
              <li v-for="curso in cursosSENCE" :key="curso"
          ...
          <div class="cyber-card bg-black/40 p-6">
            <h3 class="text-xl font-bold text-pink-400 mb-4 font-mono flex items-center gap-2">
              <span>🧠</span> SOFT SKILLS
            </h3>
            <div class="flex flex-wrap gap-2">
              <span v-for="skill in softSkills" :key="skill"
                    class="px-3 py-1 bg-pink-500/10 text-pink-300 border border-pink-500/30 rounded-full text-sm">
                {{ skill }}
              </span>
            </div>
          </div>
      ...
      <!-- Stack detallado -->
      <div class="cyber-card bg-black/40 p-6">
        <h3 class="text-xl font-bold text-cyan-400 mb-4 font-mono">// STACK DETALLADO</h3>
